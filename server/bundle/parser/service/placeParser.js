                ...
                q.push(function(next) {
                    ...
                    request.get({
                        ...
                        $('div.system .action').each(function() {
                            ...
                            // Owner
                            var owner = $this.html().match(/<a href="http:\/\/game\.asylamba\.com\/s7\/embassy\/player-(\d+)"[^>]*>([^<]+)<\/a>/);
                            if(owner) {
                                data.owner = {
                                    id: owner[1],
                                    name: owner[2]
                            ...
                            // Habitable places
                            var placeId = $this.next().html().match(/http:\/\/game\.asylamba\.com\/s7\/action\/a-[^\/]+\/commanderid-\{id}\/placeid-(\d+)\//);

                            // Non habitable places
                            if(!placeId) {
                                placeId = $this.next().html().match(/http:\/\/game\.asylamba\.com\/s7\/action\/a-createmission\/rplace-\d+\/rtarget-(\d+)\//);
                            }
