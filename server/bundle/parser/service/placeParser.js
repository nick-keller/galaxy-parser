...
function parse(sessid, relatedplace, callback) {
    ...
    request.get({
        url: 'http://game.asylamba.com/s9/bases',
        headers: {
        ...
        System.find({}).sort({ dist: 1 }).exec(function(err, systems) {
            ...
            systems.forEach(function(system, i) {
                q.push(function(next) {
                    ...
                    request.get({
                        url: 'http://game.asylamba.com/s9/ajax/a-loadsystem/systemid-' + system._id + '/relatedplace-' + relatedplace,
                        headers: {
                            Accept: '*/*',
                        ...
                        $('div.system .action').each(function() {
                            ...
                            // Owner
                            var owner = $this.html().match(/<a href="http:\/\/game\.asylamba\.com\/s9\/embassy\/player-(\d+)"[^>]*>([^<]+)<\/a>/);
                            if(owner) {
                                data.owner = {
                                    id: owner[1],
                                    name: owner[2]
                            ...
                            // Habitable places
                            var placeId = $this.next().html().match(/http:\/\/game\.asylamba\.com\/s9\/action\/a-[^\/]+\/commanderid-\{id}\/placeid-(\d+)\//);

                            // Non habitable places
                            if(!placeId) {
                                placeId = $this.next().html().match(/http:\/\/game\.asylamba\.com\/s9\/action\/a-createmission\/rplace-\d+\/rtarget-(\d+)\//);
                            }
